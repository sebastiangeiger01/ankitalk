...
export const GET: RequestHandler = async ({ params, url, platform, locals }) => {
	...
	// The four aggregates are independent — run them concurrently instead of paying
	// four serial D1 round trips.
	const [stateBreakdown, dailyReviews, retention, settingsRow] = await Promise.all([
		...
		// 3. Retention rate over reviews of mature cards. Uses the card's state AT REVIEW
		// TIME (prev_fsrs_state): joining on the current state re-classified a card's whole
		// history whenever it lapsed or graduated, skewing the rate.
		db
			.prepare(
				`SELECT
